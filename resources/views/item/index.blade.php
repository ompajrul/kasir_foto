...
@section('content')
    <div x-data="{ 
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($items as $item)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="p-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-bold {{ $item->jenis == 'paket' ? 'bg-blue-100 text-blue-600' : 'bg-emerald-100 text-emerald-600' }}">
        ...
        <div x-show="openModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" x-cloak>
            <div class="fixed inset-0 bg-black bg-opacity-40 transition-opacity" style="background-color: rgba(0, 0, 0, 0.5);" @click="openModal = false"></div>

            <div
                class="bg-white rounded-xl text-left shadow-2xl transform transition-all sm:max-w-lg sm:w-full p-6 relative z-[10000]">
                ...
                <form :action="editMode ? `/item/${itemData.id}` : '{{ route('item.store') }}'" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="nama_item" x-model="itemData.nama_item"
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="harga" x-model="itemData.harga"
                                    class="w-full border-2 border-gray-300 text-black bg-white rounded-lg px-3 py-2 focus:border-orange-500 outline-none"
                                    required>
                            </div>
                        </div>
                        ...
                        <div x-show="itemData.jenis === 'paket'">
                            ...
                            <input type="number" name="jumlah_foto" x-model="itemData.jumlah_foto"
                                class="w-full border-2 border-gray-300 text-black bg-white rounded-lg px-3 py-2 focus:border-orange-500 outline-none">
                    ...
                    <div class="mt-8 flex justify-end space-x-3">
                        <button type="button" @click="openModal = false"
                            class="px-4 py-2 text-gray-500 font-bold hover:text-gray-700 transition">Batal</button>
                        <button type="submit"
                            class="bg-orange-500 hover:bg-orange-600 text-white px-8 py-2 rounded-lg font-bold shadow-lg transition">
                            Simpan
                        </button>
                    </div>
                </form>
